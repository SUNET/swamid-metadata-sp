...
function parseJson($json) {
	...
	if ($results = json_decode($json,true)) {
		if (isset($results['objects'])) {
			$resultHandler = $db->prepare('INSERT INTO TestResults (`entityID`, `test`, `time`, `result` ) VALUES (:EntityID, :Test, :Time, :Result) ON DUPLICATE KEY UPDATE `time` = :Time, `result` = :Result');
